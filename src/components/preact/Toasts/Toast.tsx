...
const Toast: FunctionComponent<ToastProps> = ({ type, message, onClose }) => {
  const toastTypes = {
    success: {
    ...
    warning: {
      classes: 'text-orange-500 bg-orange-100 dark:bg-orange-700 dark:text-orange-200',
  ...
  const { classes, icon } = toastTypes[type];

  return (
    <div class={`flex items-center item-end w-full max-w-xs p-4 mb-4 text-gray-500 bg-white rounded-lg shadow dark:text-gray-400  `}>
      <div class={`inline-flex items-center justify-center flex-shrink-0 w-8 h-8 ${classes}`}>
        <svg class="w-5 h-5" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20" dangerouslySetInnerHTML={{ __html: icon }} />
        <span class="sr-only">{type} icon</span>
      </div>
      <div class=" mx-2 ms-3 gap-1 text-sm font-normal">{message}</div>
      <button type="button" class="ms-auto -mx-1.5 -my-1.5 bg-white text-gray-400 hover:text-gray-900 rounded-lg focus:ring-2 focus:ring-gray-300 p-1.5 hover:bg-gray-100 inline-flex items-center justify-center h-8 w-8 dark:text-gray-500 dark:hover:text-white dark:bg-gray-800 dark:hover:bg-gray-700 transition" onClick={onClose} aria-label="Close">
